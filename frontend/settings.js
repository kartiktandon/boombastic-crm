...
function personEditor(person = null) {
  ...
  modal(editing ? "Edit Person" : "Add Person", `
    ...
    async data => {
      ...
      say(editing ? "Person updated" : "Person added to your team");
      await settingsPage();
    }
  );
}
...
async function settingsPage() {
  ...
  const [s, people] = await Promise.all([api("/settings/me"), api("/people/")]);
  state.people = people;
  ...
  document.querySelector(".settings-loading").outerHTML = `<div class="settings-head"><h1>Settings</h1><p>Manage your CRM preferences and workspace.</p></div><form id="settings-form"><div class="settings-layout">
    <nav class="settings-tabs" aria-label="Settings sections">
      <button type="button" class="${active("business")}" data-settings-tab="business">▣ <span>Business Profile</span></button>
      <button type="button" class="${active("people")}" data-settings-tab="people">♙ <span>People</span></button>
      <button type="button" class="${active("leads")}" data-settings-tab="leads">◎ <span>Lead Management</span></button>
      <button type="button" class="${active("notifications")}" data-settings-tab="notifications">♧ <span>Notifications</span></button>
      <button type="button" class="${active("security")}" data-settings-tab="security">⌾ <span>Data & Security</span></button>
    </nav>
  ...
  document.querySelectorAll("[data-remove-person]").forEach(button => button.onclick = async () => {
    ...
    if (!person || !confirm(`Remove ${person.name} from assignable people?`)) return;
    button.disabled = true;
    try {
      await api(`/people/${person._id}`, {method:"DELETE"});
      activeSettingsTab = "people";
      say("Person removed");
      await settingsPage();
    } catch (error) {
  ...
  document.querySelector("#settings-form").onsubmit = async event => {
    ...
    const form = event.currentTarget, data = new FormData(form), save = form.querySelector('[type="submit"]');
    const defaultAssignment = assignmentFields(String(data.get("default_lead_owner") || ""));
    const payload = {business_name:data.get("business_name"),business_type:data.get("business_type"),business_email:data.get("business_email")||null,business_phone:data.get("business_phone")||null,website:data.get("website")||null,address:data.get("address")||null,timezone:data.get("timezone"),default_lead_owner:defaultAssignment.assigned_to,auto_assign_leads:data.has("auto_assign_leads"),followup_reminder_minutes:Number(data.get("followup_reminder_minutes")),inactivity_days:Number(data.get("inactivity_days")),weekday_start:data.get("weekday_start"),weekday_end:data.get("weekday_end"),weekend_start:data.get("weekend_start"),weekend_end:data.get("weekend_end"),notify_new_leads:data.has("notify_new_leads"),notify_followups:data.has("notify_followups"),notify_bookings:data.has("notify_bookings")};
    save.disabled = true;
    try { await api("/settings/me", {method:"PUT",body:JSON.stringify(payload)}); say("Settings saved successfully"); }
