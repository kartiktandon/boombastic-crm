function followupTabRow(lead, completed = false) {
  ...
  return `<tr>
    ...
    <td>${esc(lead.phone || "—")}</td><td>${esc(leadEventType(lead))}</td>
    ...
    <td>${esc(lead.assigned_to || "Admin User")}</td>
...
followups = async function () {
  ...
  shell(`<div class="follow-head"><div><h1>Follow-ups</h1><p>Stay on top of every conversation.</p></div></div>
    ...
    <div class="follow-layout"><div class="follow-table-wrap"><table class="follow-table"><thead><tr><th>Customer</th><th>Contact</th><th>Event Type</th><th>Follow-up Date & Time</th><th>Assigned To</th><th>Action</th></tr></thead><tbody id="follow-rows"></tbody></table></div><aside class="schedule-side"><div class="mini-calendar"><header><b>${now.toLocaleDateString("en-US", {month:"long",year:"numeric"})}</b></header><div class="week">S M T W T F S</div><div class="days">${Array.from({length:35},(_,i) => `<span class="${i+1 === now.getDate() ? "today" : ""}">${i < 31 ? i+1 : ""}</span>`).join("")}</div></div><div class="today-list"><h3>Today's Schedule</h3>${groups.today.slice(0,5).map(lead => `<div><time>${new Date(lead.next_follow_up).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}</time><span><b>${esc(lead.name)}</b><small>${esc(leadEventType(lead))}</small></span></div>`).join("") || `<p class="empty">Nothing due today.</p>`}</div></aside></div></section>`);

  const draw = () => {
    const rows = groups[active].filter(lead => [lead.name,lead.phone,leadEventType(lead)].some(value => String(value || "").toLowerCase().includes(query)));
    document.querySelector("#follow-rows").innerHTML = rows.length ? rows.map(lead => followupTabRow(lead, active === "completed")).join("") : `<tr><td colspan="6" class="empty">No ${active} follow-ups found.</td></tr>`;
